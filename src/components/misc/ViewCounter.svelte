<script lang="ts">
	...
	const getGlobalShare = () => {
		const win = window as Window & { [GLOBAL_KEY]?: { token: string | null; websiteId: string | null; promise: Promise<{ websiteId: string; token: string }> | null; statsCache: Map<string, Promise<{ pageviews: number; visitors: number; visits: number }>> } };
		if (!win[GLOBAL_KEY]) {
			win[GLOBAL_KEY] = {
				token: null,
				websiteId: null,
	...
	const getShare = (): Promise<{ websiteId: string; token: string }> => {
		...
		return (share.promise = fetch(`${config.apiBase}/share/${config.shareId}`)
			.then((res) => (res.ok ? res.json() : Promise.reject(new Error(`share ${res.status}`))))
			.then((data) => {
				share.websiteId = config.websiteId || data.websiteId;
				share.token = data.token;
				share.promise = null;
				return { websiteId: share.websiteId!, token: share.token! };
			})).catch((err) => {share.promise = null; throw err});
	};
	...
	const fetchStats = (p?: string | null) => {
		...
		if (share.statsCache.has(key)) return share.statsCache.get(key)!;
		...
		const promise = getShare()
			.then((s) => {
				...
				return fetch(`${config.apiBase}/websites/${s.websiteId}/stats?${params}`, {
	...
	const load = async () => {
		try {
			...
			const m: Metric = (metric === "pageviews" || metric === "visitors" || metric === "visits") ? metric : "pageviews";
